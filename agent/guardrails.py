...
def validate(action: dict) -> dict | None:
    ...
    if t in ("send_code_drop", "send_individual_code"):
        ...
        if acts >= state.effective_caps(game)["code_actions_per_game_per_day"]:
    ...
    if t == "ship_level":
        ...
        if shipped >= state.effective_caps(game)["levels_per_game_per_day"]:
    ...
    if t in ("send_code_drop", "send_individual_code", "send_level_push", "ship_level"):
        ...
        if pushes >= state.effective_caps(game)["push_actions_per_game_per_4h"]:
...
def doors_open(game: str) -> bool:
    ...
    if (
        ...
        < state.effective_caps(game)["levels_per_game_per_day"]
    ):
        return True
    if (
        ...
        < state.effective_caps(game)["code_actions_per_game_per_day"]
    ):
        return True
