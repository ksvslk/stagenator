...
def build_workflow(
    prompt: str, difficulty: float, solution_file_name: str = "1.png"
) -> dict:
    ...
    return {
        ...
        "15": {
            "class_type": "CLIPTextEncode",
            "inputs": {
                "text": "worst quality poor details unrealistic",
                "clip": ["10", 1],
            },
        },
        "16": {
        ...
        "17": {
            ...
            "inputs": {"image": solution_file_name, "upload": "image"},
        },
        "18": {
            "class_type": "KSampler",
            "inputs": {
                "seed": random.randint(0, 10**12),
                "steps": 20,
                "cfg": 8,
                "sampler_name": "euler",
                "scheduler": "normal",
        ...
        "20": {
            ...
            "inputs": {"samples": ["18", 0], "vae": ["10", 2]},
        },
        "21": {
            "class_type": "SaveImage",
            "inputs": {"filename_prefix": "ComfyUI", "images": ["20", 0]},
        },
    }
...
def _extract_image(status_data: dict) -> str:
    ...
    if isinstance(output, dict):
        ...
        # NOTE: output["message"] is an error string, NOT image data — never return it here
        # (it would be base64-decoded into a corrupt PNG). A message-only result -> "" -> clean raise.
...
def generate_puzzle(
    ...
    while time.monotonic() < deadline:
        ...
        ) as e:  # transient 5xx/timeout — keep polling, don't burn the job
            log.warning("runpod status poll transient error (job %s): %s", job_id, e)
            continue
        ...
        if status in ("FAILED", "CANCELLED"):
            raise RuntimeError(
                f"runpod job {job_id} {status}: {data.get('error') or data}"
            )
    raise RuntimeError(f"runpod job {job_id} timed out after {timeout_s}s")


def account_balance() -> float | None:
    # The run key (RUNPOD_API_KEY) may be endpoint-scoped and unable to read the
    # account; RUNPOD_BALANCE_KEY (an account-read key) takes precedence for balance.
    key = os.getenv("RUNPOD_BALANCE_KEY") or os.getenv("RUNPOD_API_KEY")
    if not key:
        log.warning("no Runpod key set — balance unknown")
        return None
    q = {"query": "query { myself { clientBalance } }"}
    try:
        r = requests.post(
            GRAPHQL_API, json=q, headers={"Authorization": f"Bearer {key}"}, timeout=30
        )
        ...
        Exception
    ) as e:  # transient 5xx / network / parse — never crash the balance task
