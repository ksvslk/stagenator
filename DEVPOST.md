...
- **Development & Testing:** Developed spec-first using Google's ADK skills (agents-cli-workflow, adk-code, deploy, eval, scaffold), driven by Claude Code as the coding agent — the skills are coding-agent-agnostic by design, and every commit in the repo carries its co-author trailer. Verified with 51 unit tests, 28 Firestore emulator resilience tests, and a graded agents-cli eval benchmark (passing 4/4 test scenarios).
...
5. **Difficulty You Can't Set Directly (Subliminal Words):**
   For a hidden-word puzzle, "difficulty" means *how hard the word is to spot* — but the only knob we pass to the image model (Runpod / ControlNet) is a raw conditioning **strength**: turn it up and the letters are stamped boldly onto the scene (easy to find); ease it down and they melt into it (hard). A single fixed value is wrong for every level, because a layout of huge bold letters is already easy while small thin letters are already hard. So we made the strength **adaptive**: deterministic code reads the letter layout it just generated and sets the strength *inversely* to how visible the letters already are, compensating so the real find-difficulty stays even across wildly different words. The honest lesson from generating actual images: the strength knob only bites when letters must hide in *incidental* structure — foliage, rock, trees. When the scene naturally offers a surface to write on (sand, snow, clouds), the model renders the word readably no matter the number, so there the *scene choice*, not the strength, is the true difficulty lever. Gemini vision QA still gates every level for readability before it ships.
...
But the direction I'm most excited about is **personalization that compounds with the crowd**. Every pulse, the agent already sees signals it *could* act on — which country the active players are in, whether they're brand-new or returning. In principle this is gold: culturally resonant scenes for a given country, gentler onboarding levels for a first-timer, fresh variety for someone coming back. And here's the flywheel — **the more players show up, the more interesting it gets and the better the AI should get at it**: more traffic means more geography, more A/B signal, more return patterns, which is exactly the fuel an agent needs to *keep getting better instead of merely running consistently*. Today, on near-zero traffic, we've taken only the smallest honest step — an **optional, soft geo hint** the level designer *may* honor but is explicitly told never to force. The real prize is giving the AI genuine creative freedom to tailor content per audience *without* breaking the determinism and guardrails that keep it safe (*code does the doing, AI does bounded thinking*). We haven't found the perfect way to let the model vary on these signals yet — it's the open problem we're most actively chewing on, and the one that grows more valuable with every extra player who arrives.
