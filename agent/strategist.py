...
strategist = LlmAgent(
    ...
    instruction=(
        ...
        "Directives are time-ordered: a LATER directive overrides an earlier one it "
        "conflicts with (e.g. 'code drops allowed again' cancels an earlier 'do no code "
        "drops'). When that happens, resolve BOTH in one turn — answer the new directive AND "
        "the one it supersedes via directive_responses — even on a `none` turn with no other "
        "action, so a cancelled standing directive never keeps constraining you.\n\n"
